...
import type { InterestArea, Profile } from "@/sanity/lib/types";
import { Microchip } from "lucide-react";
import Image from "next/image";
import { type ReactNode } from "react";
import profile_image from "../app/assets/jhony.png";
import InterestAreas from "./interestCircle";
...
const INTEREST_ICONS: Record<string, ReactNode> = {
  "Software Architecture": <SaasIcon size={40} className="fill-green" />,
  "Artificial Intelligence": <IAcon size={40} className="fill-purple" />,
  "IoT & Embedded Systems": <Microchip size={40} className="stroke-yellow" />,
};
            ...
            <div className="flex items-center gap-3">
              ...
              <Tooltip label="Curriculum">
                {(profile.cvFileUrl ?? profile.cvUrl) ? (
                  <a
                    href={(profile.cvFileUrl ?? profile.cvUrl)!}
                    target="_blank"
                    rel="noopener noreferrer"
        ...
        <div className="flex items-center flex-col md:justify-evenly md:flex-row gap-8 relative">
          {interests.map((area) => (
            <InterestAreas
              ...
              title={area.name}
              className={
                INTEREST_CLASS[area.name] ?? "border border-border-color"
              }
